...
describe('pelvic placements', () => {
    it('adds a pelvic placement at S2AI level with left zone', () => {
        const state = createInitialState();
        const p = { id: 'pv1', levelId: 'S2AI', zone: 'left', tool: 'polyaxial', data: '7.0x50', annotation: '' } as Placement;
        const next = documentReducer(state, { type: 'ADD_PLACEMENT', chart: 'plan', placement: p });
        expect(next.plannedPlacements).toHaveLength(1);
        expect(next.plannedPlacements[0].zone).toBe('left');
        expect(next.plannedPlacements[0].levelId).toBe('S2AI');
        expect(next.plannedPlacements[0].data).toBe('7.0x50');
    });

    it('allows pelvic levels (S2AI, Iliac, SI-J) to coexist with S1', () => {
        const state = createInitialState();
        const pS1 = { id: 'pv1', levelId: 'S1', zone: 'left', tool: 'polyaxial', data: '6.5x45', annotation: '' } as Placement;
        const pS2ai = { id: 'pv2', levelId: 'S2AI', zone: 'left', tool: 'polyaxial', data: '7.0x50', annotation: '' } as Placement;
        const pIliac = { id: 'pv3', levelId: 'Iliac', zone: 'left', tool: 'polyaxial', data: '7.5x80', annotation: '' } as Placement;
        const pSI = { id: 'pv4', levelId: 'SI-J', zone: 'left', tool: 'polyaxial', data: '7.0x60', annotation: '' } as Placement;
        let s = documentReducer(state, { type: 'ADD_PLACEMENT', chart: 'plan', placement: pS1 });
        ...
        const levelIds = s.plannedPlacements.map(p => p.levelId);
        expect(levelIds).toContain('S1');
        expect(levelIds).toContain('S2AI');
        expect(levelIds).toContain('Iliac');
        expect(levelIds).toContain('SI-J');
    });

    it('enforces one-per-zone for pelvic levels', () => {
        const state = createInitialState();
        const p1 = { id: 'pv1', levelId: 'S2AI', zone: 'left', tool: 'polyaxial', data: '7.0x50', annotation: '' } as Placement;
        const p2 = { id: 'pv2', levelId: 'S2AI', zone: 'left', tool: 'monoaxial', data: '7.0x55', annotation: '' } as Placement;
        const s1 = documentReducer(state, { type: 'ADD_PLACEMENT', chart: 'plan', placement: p1 });
        const s2 = documentReducer(s1, { type: 'ADD_PLACEMENT', chart: 'plan', placement: p2 });
        // Pelvic levels now use standard left/right zones — one-per-zone guard applies
        expect(s2.plannedPlacements).toHaveLength(1);
        expect(s2).toBe(s1); // same reference — no change
    });
    ...
    it('removes a pelvic placement by id', () => {
        const state = createInitialState();
        const p = { id: 'pv1', levelId: 'Iliac', zone: 'left', tool: 'polyaxial', data: '7.5x80', annotation: '' } as Placement;
        const s1 = documentReducer(state, { type: 'ADD_PLACEMENT', chart: 'plan', placement: p });
