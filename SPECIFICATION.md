...
| UR-02 | Instrument types must include pedicle screws (monoaxial, polyaxial, uniplanar), hooks (pedicle, TP down, TP up, supra-laminar, infra-laminar), bands, wires, cables, and midline devices (crosslinks, osteotomies) | Reflects the instrumentation vocabulary used in spinal deformity surgery |
...
| UR-34 | Screws and hooks must support free-text annotations (e.g. "fenestrated, cortical, revision") | Documents technique or implant variant without adding rigid categories |
| UR-35 | Bands, wires, and cables must support free-text descriptions | Documents fixation details (e.g. "Atlas titanium cable") |
| UR-04 | Osteotomies must be classifiable by Schwab grade (1–6) or as Corpectomy, with a correction angle | Schwab classification is the standard grading system for spinal osteotomies |
| UR-36 | VCR, ML-VCR, and Corpectomy osteotomies must support a reconstruction cage text field | Documents the cage/device used to reconstruct the anterior column |
...
| UR-31 | Implant supplier and screw system must be selectable | Identifies the manufacturer and product line for the construct |
...
| UR-37 | Bone graft must be recordable with multi-select types and free-text notes | Documents graft material used in the procedure |
...
| UR-18 | A Session Privacy Mode must exist that disables all local storage | NHS shared workstations must not retain patient data (GDPR / Caldicott) |
...
|----|-------------|----------------|
| TR-01 | Single-file HTML application | All markup, styles, and scripts in one `index.html` (~1574 lines, ~95 KB) |
...
| TR-07 | Persistence via browser localStorage | Key: `spine_planner_v2`; stores full JSON v3 payload via shared `serializeState()` / `deserializeState()` |
...
| TR-09 | Instruments rendered as SVG icons within vertebral zones | InstrumentIcon component with per-tool SVG rendering; screws/hooks as SVG paths, band/wire/cable as bold text SVGs |
| TR-10 | Export canvas must maintain fixed dimensions regardless of screen size | Reference dimensions 1485 x 1050 px, 3 columns: patient info (340px) + Plan (flex-4) + Construct (flex-3). Isotropic scaling via ResizeObserver + CSS transform |
| TR-11 | View mode scaling must auto-scale to fit available height | `calculateAutoScale` computes height ratio, capped at 1.5x |
...
| ID | Constraint | Rule |
|----|-----------|------|
| TR-13 | Screws, hooks, bands, wires, and cables may only be placed in `left` or `right` zones | One implant per side per level |
...
```
{
  ...
  tool:       string    // tool identifier (e.g. "polyaxial", "pedicle_hook", "osteotomy", "band")
  data:       mixed     // screw: "6.5x45" | osteotomy: {type, angle, shortLabel, reconstructionCage} | null (hooks) | string (band/wire/cable description)
  annotation: string    // free text for screws/hooks (e.g. "fenestrated, cortical"); empty string if unused
}
```
...
```
{
  ...
  boneGraft:   object    // { types: string[], notes: string }
}
```
...
Stores the full JSON v3 structure including `timestamp`. Shared `serializeState()` / `deserializeState()` functions ensure auto-save, file save, auto-load, and file load all use identical logic. Loads files with `formatVersion >= 2` for backward compatibility.

### 3.5 Implant Type Constants

**HOOK_TYPES:** `['pedicle_hook', 'tp_hook', 'tp_hook_up', 'sl_hook', 'il_hook']`
**NO_SIZE_TYPES:** `[...HOOK_TYPES, 'band', 'wire', 'cable']` — these types have no diameter/length sizing

**Tool categories in ScrewModal:**
| Tier | Tools | Visual Weight | Sizing |
|------|-------|---------------|--------|
| SCREWS | monoaxial, polyaxial, uniplanar | Largest (py-2.5, w-6 h-6 icons) | Diameter + Length |
| HOOKS | pedicle, TP (Down), TP (Up), supra-lam, infra-lam | Medium (py-1.5, w-4 h-4 icons) | None — "Hook placement — no sizing." |
| BANDS & OTHERS | band, wire, cable | Compact (py-1, text labels) | None — free-text description field |

**Annotations:** Available for screws and hooks (not bands/wires/cables which have descriptions instead). Displayed below size/type label on diagram.

### 3.6 Cage Permissibility
...
T11    |  -   | Yes  | Yes  | Yes  |  -   |  -
T12    |  -   | Yes  | Yes  | Yes  | Yes  |  -
L1-L3  |  -   | Yes  | Yes  | Yes  | Yes  |  -
L4     |  -   | Yes  | Yes  | Yes  | Yes  | Yes
L5     |  -   | Yes  | Yes  |  -   |  -   | Yes
```

Note: XLIF permissibility is T5-L4 in the code (reflecting clinical use of thoracolumbar XLIF), though standard teaching starts at T11.

### 3.7 Osteotomy Types

Grouped in modal by approach:

**Posterior (Schwab Classification):**
- Facet (Grade 1)
- Facet + ALL (Grade 2)
- Pedicle/Wedge (PSO, Grade 3)
- Pedicle + Disc (Grade 4)
- VCR (Grade 5) — shows reconstruction cage field
- ML-VCR (Grade 6) — shows reconstruction cage field

**Anterior:**
- Corpectomy — shows reconstruction cage field; renders dashed vertebral body outline

### 3.8 Bone Graft

Options: Local Bone, Autograft, Allograft, Synthetics, DBM, BMP

Stored in `patientData.boneGraft: { types: string[], notes: string }`. Displayed as 3-column checkbox grid with free-text notes field. Backward compatible — missing `boneGraft` property treated as `{ types: [], notes: '' }`.

### 3.9 Colour Schemes

7 sidebar-only themes (printed form unaffected):

| ID | Label | Mapped Company |
|----|-------|----------------|
| default | Slate & Amber | — |
| blue | NHS Blue | — |
| navy | Deep Navy | Medtronic |
| gold | Gold & Black | Stryker |
| red | Red | DePuy Synthes |
| purple | Purple | VB Spine |
| midnight | Midnight | Globus Medical |
...
| TR-20 | Session Privacy Mode clears localStorage after export | Defence-in-depth for GDPR compliance on shared machines |

### 3.11 Browser Compatibility
...
### 3.12 Licensing and Distribution
...
|----|-------------|--------|
| FR-03 | Interbody cage support (ACDF, PLIF, TLIF, XLIF, OLIF, ALIF) | **Implemented** (v0.7.0) |
...
| FR-17 | **Company and screw system selection** with auto-theme | **Implemented** (v0.8.2) |
| FR-18 | **Colour scheme system** with company auto-switching | **Implemented** (v0.8.2, labels updated v0.9.0) |
| FR-08 | **Additional screw types** (fenestrated, cortical, cannulated, revision, S2AI) | Partially addressed — annotation field captures these as free text (v0.9.0) |
| FR-09 | **Per-implant notes** (free text on all implant modals) | **Implemented** (v0.9.0) — annotations for screws/hooks, descriptions for fixation |
| FR-10 | **Bone graft recording** (autograft, allograft, BMP, synthetic, with location) | **Implemented** (v0.9.0) |
| FR-11 | **Wire/cable/band recording** (sublaminar wires, cables, bands) | **Implemented** (v0.9.0) |
| FR-12 | **Enhanced VBR recording** (expandable/static/mesh, sizing) | Partially addressed — reconstruction cage text field on VCR/Corpectomy (v0.9.0) |
| FR-19 | **Anatomically accurate vertebrae** — pedicle width/height and vertebral body dimensions from measured data | Planned (data collection in progress) |
...
| FR-14 | Manufacturer/catalogue fields and GUDID lookup | Partially implemented — company/screw system fields; GUDID lookup not started |
