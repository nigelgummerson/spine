...
- **v2.7.35-beta** (2026-03-28): Version bump — screw trajectory selector, screw shanks on chart, corrected pedicle positions, set screw/blocker/locking cap inventory, grouped note presets, expanded keyboard shortcuts and onboarding tour, export format picker, spinal level range in inventory, schematic notice on chart.
...
- **v2.7.32-beta** (2026-03-26): 5-agent expert review (architect, safety, accessibility, QA, i18n) — 68 findings, 29-item phased plan. Record lock enforced at reducer level (all CRUD rejected when locked, sync blocked on locked records). useDocumentState split into useBroadcastSync + useAutoSave + composition layer. useModalState extracted from App.tsx (21 useState). ErrorBoundary wraps ModalOrchestrator. Disclaimer acceptance recorded in document state. OsteotomyModal blocks at Oc/C1/C2. NEW_PATIENT broadcasts immediately. peerIncognitoRef resets on heartbeat timeout. RTL_LANGUAGES cleaned. Schema: 15 .passthrough() replaced with .strict(), defensive numeric validation. 14 useCallback wrappers for ChartPaper props. ~15 `any` types eliminated. formatDate() via Intl.DateTimeFormat. 3 advisory glossary CI tests. 100 new tests (export pipeline, lock enforcement, sync, privacy, cage/osteotomy permissibility, checksum, screw defaults, chart stress, utilities). 1273 tests across 43 files.
