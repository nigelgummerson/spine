...
1. Tag the commit: `git tag v2.7.3-beta` (must match pattern `v*.*.*` or `v*.*.*-*`)
2. Push the tag: `git push origin v2.7.3-beta`
...
2. Edit CLAUDE.md                       # version header + technical history entry
...
9. git tag v2.7.X-beta && git push origin v2.7.X-beta  # triggers standalone release
